...
El Oráculo del Sabor
Auténtica Gastronomía Griega en Ecatepec
...
ACCESO STAFF
            ...
            <div class="row g-4">
                <div class="col-md-4">
                    <div class="glass-card h-100">
                        ...
                        <h4 class="text-white">Gyros de Pollo</h4>
                        <p class="text-white-50">Válido todos los Martes y Jueves.</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="glass-card h-100">
                        ...
                        <p class="text-white-50">Presenta tu credencial universitaria vigente.</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="glass-card h-100">
                        <div class="promo-badge">GRATIS</div>
                        <h4 class="text-white">Postre Baklava</h4>
                        <p class="text-white-50">En tu primera reserva desde la web.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section id="ubicacion" class="container py-5 text-center">
        ...
        <p class="text-white-50">Multiplaza Aragón, Av. Central, Ecatepec</p>
        <div class="glass-card p-0 overflow-hidden" style="height: 400px;">
            <iframe width="100%" height="100%" style="border:0;" 
                src="[https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3760.40488661642!2d-99.03056082405786!3d19.524259837136006!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x85d1fb1b90c91839%3A0xc3b53f66838a164a!2sMultiplaza%20Arag%C3%B3n!5e0!3m2!1ses-419!2smx!4v1714100000000!5m2!1ses-419!2smx](https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3760.40488661642!2d-99.03056082405786!3d19.524259837136006!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x85d1fb1b90c91839%3A0xc3b53f66838a164a!2sMultiplaza%20Arag%C3%B3n!5e0!3m2!1ses-419!2smx!4v1714100000000!5m2!1ses-419!2smx)" 
...
snap.forEach(doc => {
const p = doc.data();
container.innerHTML +=  <div class="col-md-4"> <div class="glass-card text-center h-100"> <img src="${p.imagen}" onerror="this.src='[https://placehold.co/400x300?text=Griego](https://placehold.co/400x300?text=Griego)'" class="img-fluid rounded mb-3" style="height:200px; width:100%; object-fit:cover;"> <h4 class="text-gold">${p.nombre}</h4> <p class="small text-white-50"><i>${p.descripcion || 'Receta tradicional'}</i></p> <h5 class="fw-bold">$${p.precio}</h5> </div> </div>;
});
};
...
document.getElementById('main-content').innerHTML =  <div class="container my-5 pt-5"><div class="row justify-content-center"><div class="col-md-5"> <div class="glass-card" id="auth-box"> <h3 class="text-center mb-4 text-gold">${esPersonal ? 'Acceso Staff' : 'Bienvenido'}</h3> <input id="auth-e" class="form-control mb-2" placeholder="Correo"> <input id="auth-p" type="password" class="form-control mb-3" placeholder="Contraseña"> <button onclick="window.handleLogin()" class="btn btn-primary w-100 mb-2">Entrar</button> ${!esPersonal ? '<button onclick="window.showRegister()" class="btn btn-outline-gold w-100 btn-sm">Crear Cuenta Cliente</button>' : ''} <button onclick="window.renderLanding()" class="btn btn-link w-100 text-white mt-3">Volver al Inicio</button> </div> </div></div></div>;
};
...
window.showRegister = () => {
document.getElementById('auth-box').innerHTML =  <h3 class="text-center mb-4 text-gold">Nuevo Cliente</h3> <input id="reg-n" class="form-control mb-2" placeholder="Nombre"> <input id="reg-e" class="form-control mb-2" placeholder="Email"> <input id="reg-p" type="password" class="form-control mb-3" placeholder="Password"> <button onclick="window.handleRegister()" class="btn btn-primary w-100">Registrarme</button>;
};
...
document.getElementById('main-content').innerHTML = ESTILOS_GLOBALES +  <div class="container my-5"> <div class="row g-4"> <div class="col-lg-7"> <div class="glass-card text-center"> <h2 class="mb-4 text-gold">Reserva tu Mesa</h2> <div class="row mb-3"> <div class="col-md-6"><input id="res-f" type="date" class="form-control" min="${new Date().toISOString().split('T')[0]}"></div> <div class="col-md-6"> <select id="res-h" class="form-select"><option>14:00</option><option>17:00</option><option>20:00</option></select> </div> </div> <input id="res-p" type="number" class="form-control mb-4" placeholder="Personas" value="2"> <div id="grid-reserva" class="grid-mesas mb-4"></div> <button id="btn-confirmar-res" class="btn btn-primary w-100 d-none" onclick="window.saveReserva()">CONFIRMAR RESERVA</button> </div> </div> <div class="col-lg-5"> <div class="glass-card"> <h4 class="text-gold mb-3 text-center">Mis Próximas Visitas</h4> <div id="lista-historial"></div> </div> </div> </div> </div> <div class="modal fade" id="modalTicket" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content bg-white text-dark p-4 text-center"> <h4 class="fw-bold">TICKET DE RESERVA</h4><hr> <div id="ticket-info"></div><hr> <button class="btn btn-dark w-100 no-print" onclick="window.print()">Imprimir / Guardar</button> </div></div></div>;
...
document.getElementById('main-content').innerHTML = ESTILOS_GLOBALES +  <div class="container my-5"> <h2 class="text-gold text-center mb-5">Panel de Gerencia</h2> <div class="row g-4"> <div class="col-md-4"> <div class="glass-card"> <h4 class="text-gold mb-3">Nuevo Platillo</h4> <select id="p-categoria" class="form-select mb-2"><option value="entrada">Entrada</option><option value="plato_fuerte">Plato Fuerte</option><option value="postre">Postre</option><option value="bebida">Bebida</option></select> <input id="p-nombre" class="form-control mb-2" placeholder="Nombre"> <textarea id="p-desc" class="form-control mb-2" placeholder="Descripción..."></textarea> <input id="p-precio" type="number" class="form-control mb-2" placeholder="Precio"> <input id="p-img" class="form-control mb-3" placeholder="URL Imagen"> <button onclick="window.guardarProducto()" class="btn btn-primary w-100">Añadir al Menú</button> </div> </div> <div class="col-md-4"> <div class="glass-card"> <h4 class="text-gold mb-3">Alta de Mesero</h4> <input id="m-nombre" class="form-control mb-2" placeholder="Nombre completo"> <input id="m-email" class="form-control mb-2" placeholder="Correo mesero"> <input id="m-pass" type="password" class="form-control mb-3" placeholder="Contraseña mesero"> <button onclick="window.crearMesero()" class="btn btn-outline-gold w-100">Registrar Mesero</button> </div> </div> <div class="col-md-4 text-center"> <div class="glass-card"> <h4 class="text-gold mb-4">Ventas del Día</h4> <h2 id="total-caja" class="display-6 text-white">$0</h2> </div> </div> </div> <div class="mt-5"> <h4 class="text-gold mb-3">Ventas Recientes</h4> <div class="glass-card table-responsive"> <table class="table table-dark"> <thead><tr><th>Fecha</th><th>Mesa</th><th>Mesero</th><th>Total</th></tr></thead> <tbody id="lista-ventas-body"></tbody> </table> </div> </div> </div>;
...
window.renderMesero = () => {
document.getElementById('main-content').innerHTML =  <div class="container my-5"> <h2 class="text-center mb-4 text-gold">Terminal de Atención</h2> <div class="row"> <div class="col-md-4"><div class="glass-card"><h4>Mesas</h4><div id="grid-mesas-m" class="grid-mesas"></div></div></div> <div id="area-atencion" class="col-md-8 d-none"> <div class="glass-card"> <h3 class="text-center">Atendiendo Mesa: <span id="m-atend" class="text-gold">--</span></h3> <select id="select-platillo" class="form-select mb-3" onchange="window.abrirModalCantidad(this.value)"><option value="" disabled selected>Agregar producto...</option></select> <div id="lista-pedido" class="mb-4"></div> <h4 class="text-end text-gold">Total: $<span id="total-atencion">0</span></h4> <button class="btn btn-primary w-100" onclick="window.generarTicketFinal()">CERRAR CUENTA</button> </div> </div> </div> </div> <div class="modal fade" id="modalCantidad" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content bg-dark text-white p-4 text-center"> <h5 id="p-nombre-modal"></h5> <div class="d-flex justify-content-center align-items-center gap-3 my-3"> <button class="btn btn-outline-gold" onclick="window.modCant(-1)">-</button> <h3 id="p-cant-modal">1</h3> <button class="btn btn-outline-gold" onclick="window.modCant(1)">+</button> </div> <button class="btn btn-primary w-100" onclick="window.confirmarProducto()">Añadir</button> </div></div></div>;
